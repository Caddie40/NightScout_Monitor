...
function prepareData(obj) {
  ...
  // CODEX local guard: Android/browser treatment forms can occasionally submit
  // a manual treatment almost exactly 24h in the future. That breaks IOB/COB
  // because insulin has not "happened" yet according to server time. If the
  // timestamp is >2h ahead, and subtracting 24h makes it plausible/recent,
  // normalize it back one day. This preserves legitimate near-future prebolus
  // entries while preventing impossible next-day corrections from hiding IOB.
  const nowForFutureGuard = moment();
  const treatmentMomentForFutureGuard = moment.parseZone(results.created_at);
  if (treatmentMomentForFutureGuard.isValid()
    && treatmentMomentForFutureGuard.diff(nowForFutureGuard, 'hours', true) > 2) {
    const minusOneDay = treatmentMomentForFutureGuard.clone().subtract(1, 'day');
    if (minusOneDay.diff(nowForFutureGuard, 'hours', true) <= 2
      && nowForFutureGuard.diff(minusOneDay, 'hours', true) <= 48) {
      console.warn('CODEX future treatment timestamp guard: shifting manual treatment back 24h from ' + results.created_at + ' to ' + minusOneDay.toISOString());
      results.created_at = minusOneDay.toISOString();
    }
  }
